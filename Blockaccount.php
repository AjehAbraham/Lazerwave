...
if (!isset($_SESSION["New_user"])){
  ...
  $result_for_history = $conn -> query($check_status_record) -> fetch_assoc();

  $status = "";

  if ($result_for_history == NULL ){

    $status = "Block Account";
    
  }else if ($result_for_history["Account_status"] == "Unblock"){
    $status = "Block Account";
  }
  
  else{
    $status = "Unblock Account";
  }
...
<form method='post' id='FormId'>
    ...
    <input type="number" name="Pin_code"  placeholder='Enter your four digit transaction pin' inputmode="numeric" style="-webkit-text-security:disc;" maxlength="4">
    <br>
