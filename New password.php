...
if (empty($new_password)){
    ...
    $message_status = "Enter password for new password";
...
if (!preg_match("/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/" ,$new_password)) 
{
  ...
  $message_status = "Password must be at least one uppercase,one lowercase,one number and one special character";
...
if (empty($confirm_password)){
  ...
  $message_status = "Confirm password cannot be empty";
...
if (!$new_password == $confirm_password){
  ...
  $message_status = "New password and confirm password must be thesame";
...
$message = "<html></body>";
$message .="<p style='background-color: rgb(0,0,180);padding: 10px 10px;color: white;margin:auto;'>Lazerwave</p>";
$message .="<p style='margin-left: 6px'> Hello>" .  $_SESSION["Surname"]. "</p>";
$message .="<p>Your otp is:</p>";
